...
class DataGenerator(BaseDataGenerator):
	...
	def _expend_dim_on_needed(self, data):
	...
	def load_NAB_dataset(self, dataset, y_scale=6):
		...
		for i in range(n_train_sample - self.config['l_win'] + 1):
			rolling_windows[i] = self._expend_dim_on_needed(data['training'][i:i + self.config['l_win']])
		...
		for k in range(self.config['l_win']):
			...
			cur_lstm_seq = np.zeros(
				(n_train_lstm, self.config['l_seq'], self.config['l_win'], self.config['n_channel']))
			for i in range(n_train_lstm):
				...
				for j in range(self.config['l_seq']):
					# print(k,i,j)
					cur_seq[j] = self._expend_dim_on_needed(data['training'][
															k + self.config['l_win'] * (j + i): k + self.config[
																'l_win'] * (j + i + 1)])
